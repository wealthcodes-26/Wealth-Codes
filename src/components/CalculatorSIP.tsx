...
const CalculatorSIP = () => {
  ...
  const results = useMemo(() => {
    ...
    const totalInvestment = monthlyVal * n;
    
    // SIP Formula: P * [((1 + i)^n - 1) / i] * (1 + i)
    const futureValue = i === 0 ? totalInvestment : monthlyVal * ((Math.pow(1 + i, n) - 1) / i) * (1 + i);
    ...
    for (let year = 1; year <= periodVal; year++) {
      ...
      const value = i === 0 ? invested : monthlyVal * ((Math.pow(1 + i, months) - 1) / i) * (1 + i);
      chartData.push({
        year,
